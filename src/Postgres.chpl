...
class PgConnection: ConnectionBase {
  ...
  proc init(host: string, user: string = "", database: string = "", passwd: string = "") {
    ...
    if (PQstatus(conn) != CONNECTION_OK) {
      var err: string = PQerrorMessage(conn): c_string;
...
class PgCursor: CursorBase {
  ...
  override proc execute(query: string) {
    ...
    if (PQresultStatus(res) !=  PGRES_COMMAND_OK) {
      var err: string = PQerrorMessage(this.pgcon): c_string;
    ...
    var ii: int(32 ) = 0;
    while ( ii < nFields) {
      var colname: string = PQfname(this.res, ii: c_int);
  ...
  override proc dump() {
    ...
    while ( i < rows) {     
      ...
      while(j < this.nFields) {
        row = PQgetvalue(res, i: c_int, j: c_int): c_string;
  ...
  proc fetchrow(idx:int): shared Row? {
    ...
    while(j < this.nFields) {
      var datum: string = PQgetvalue(res, this.curRow: c_int, j: c_int): c_string;
  ...
  override iter these()ref: shared Row {
